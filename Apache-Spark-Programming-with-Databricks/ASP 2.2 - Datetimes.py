...
# TODO
datetimeDF = (df.withColumn("ts", (col("ts")/1e6).cast("timestamp")).withColumn("date",to_date(col("ts")))
)
display(datetimeDF)
...
# TODO
activeDowDF = (activeUsersDF.withColumn("day", date_format(col("date"), "E")).groupBy("day").agg(avg("active_users").alias("avg_users"))
)
display(activeDowDF)
